...
class Timer
{
  ...
  public:
    ...
    **************************************************************************/
    void startUpTimer(int newDuration)
    {
      //IMPLEMENT
      startTime = millis();
    ...
    **************************************************************************/
    ...
    //returns true if the timer has completed its duration
...
//a timer to control the notes duration
Timer timerForNotes(MAX_PRESS_TIME);

//a timer to control the pauses between playback notes
Timer timerForPause(100);

//keeps track of which note is playing back
int currentNote = 0;
...
void reset()
{
 countNotes = 0;
 currentNote = 0;
 timerForNotes.stopTimer();
 timerForPause.stopTimer();
}
/******************LIVE()  - RE-IMPLEMENT **************************************
 * INSTRUCTIONS:
...
{
   // to handle a little bit of noise (threshold)
   int theAdjuster = 8;
  ...
  if (countNotes < MAX_NOTES)
  {
     ...
     */
    if (testNote > theAdjuster && (!timerForNotes.timerComplete())) {
      /*** STATE AA::: *******************************************
      ...
      */
      ...
      /*** FUNCTION TO IMPLEMENT ***/
        timerForNotes.startUpTimer(MAX_PRESS_TIME);
     ...
     */
    else if (testNote > theAdjuster && (timerForNotes.timerComplete()))
    {
...
void updateArraysWithNoteAndTimings()
{
  ...
  if(countNotes < MAX_NOTES)
  {
    notes[countNotes] = analogRead(NOTE_IN_PIN) + getRunningAverage();
    if(timerForNotes.getDuration() < MAX_PRESS_TIME)
    {
      durations[countNotes] = timerForNotes.getDuration();
    }
    else
    {
  ...
  timerForNotes.startUpTimer(durations[currentNote]);
}
...
void playWithDuration()
{
  ...
  if(timerForNotes.timerComplete())
  {
    //checks if the timer has been started yet and if it hasn't then start it
    if(timerForPause.timerComplete())
    {
      timerForPause.startUpTimer(100);
    }
    
    timerForPause.updateTimer();
